...
Outputs (per k, psf; grouped under results_resolution/k{K}_psf{P}/):
  - results_resolution/k{K}_psf{P}/scores_k{K}_psf{P}.csv  — per-target test-set scores
  - results_resolution/k{K}_psf{P}/metrics_k{K}_psf{P}.txt — per-fold + ensemble metrics
  - results_resolution/k{K}_psf{P}/confusion_matrix_k{K}_psf{P}.png
"""
...
def _train_and_eval_one_csv(csv_path: str) -> None:
    ...
    # Per-(k, psf) training set outputs are grouped under their own subfolder
    # (e.g. results_resolution/k1_psf0/) to avoid dumping every result into
    # results_resolution/ directly.
    out_subdir = os.path.join(OUTPUT_DIR, tag)
    ...
    scores_path = os.path.join(out_subdir, f"scores_{tag}.csv")
    ...
    save_confusion_matrix(
        ...
        os.path.join(out_subdir, f"confusion_matrix_{tag}.png"),
    )
    save_metrics_report(
        fold_metrics, ensemble_metrics, n_total,
        os.path.join(out_subdir, f"metrics_{tag}.txt"),
        tag=tag,
